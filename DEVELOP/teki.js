...
function tekiMove03(obj) {
    if(!obj.flag && (obj.y>>8)>=140)obj.flag=1;
    if(obj.flag==1)
    {
        if((obj.vx-=2)<=0)
        {
    ...
    if(obj.hp<obj.mhp/2)
    {
        ...
        {
            let an, dx, dy;
            an = (90+45-(c/10)*30) * Math.PI/180;  //ラジアン
            dx = Math.cos(an) * 300;
            dy = Math.sin(an) * 300;
            let x2 = (Math.cos(an)*70)<<8;
            let y2 = (Math.sin(an)*70)<<8;
            teki.push(new Teki(3, obj.x+x2, obj.y+y2, dx, dy,60));
        }
    }
...
function tekiMove01(obj) {
    if (!obj.flag) {
        //自機の方に向かう感じ
        if (jiki.x > obj.x && obj.vx < 120) obj.vx += 1;
        else if (jiki.x < obj.x && obj.vx > -120) obj.vx -= 1;
    }
    else {
        //自機から逃げる感じ
        if (jiki.x < obj.x && obj.vx < 400) obj.vx += 5;
        else if (jiki.x > obj.x && obj.vx > -400) obj.vx -= 5;
    }
...
function tekiMove02(obj) {
    if (!obj.flag) {
        //自機の方に向かう感じ
        if (jiki.x > obj.x && obj.vx < 600) obj.vx += 2;
        else if (jiki.x < obj.x && obj.vx > -600) obj.vx -= 2;
    }
    else {
        //自機から逃げる感じ
        if (jiki.x < obj.x && obj.vx < 600) obj.vx += 7;
        else if (jiki.x > obj.x && obj.vx > -600) obj.vx -= 7;
    }
